use gpui::{
    FontStyle, HighlightStyle, Hsla, SharedString, StrikethroughStyle, UnderlineStyle, px, rgba,
};
use std::ops::Range;
use theme::Appearance;

pub const MAX_YMD_HIGHLIGHT_BYTES: usize = 100_000;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum YmdHighlightKind {
    Background(YmdColor),
    LineForeground(YmdColor),
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct YmdHighlight {
    pub range: Range<usize>,
    pub kind: YmdHighlightKind,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct YmdConceal {
    pub range: Range<usize>,
}
...
pub struct YmdStyledHeading {
    pub text: SharedString,
    pub foreground_style: HighlightStyle,
    visible_source_ranges: Vec<Range<usize>>,
    source_len: usize,
}

impl YmdStyledHeading {
    pub fn rendered_range_for_source_range(&self, range: Range<usize>) -> Option<Range<usize>> {
        let mut rendered_offset = 0;
        let mut rendered_range = None;

        for visible_source_range in &self.visible_source_ranges {
            let start = range.start.max(visible_source_range.start);
            let end = range.end.min(visible_source_range.end);
            if start < end {
                let rendered_start = rendered_offset + start - visible_source_range.start;
                let rendered_end = rendered_offset + end - visible_source_range.start;
                rendered_range = Some(
                    rendered_range
                        .map(|range: Range<usize>| range.start.min(rendered_start)..rendered_end)
                        .unwrap_or(rendered_start..rendered_end),
                );
            }

            rendered_offset += visible_source_range.len();
        }
...
pub fn background_style(color: YmdColor, appearance: Appearance) -> HighlightStyle {
    HighlightStyle {
...
pub fn link_style() -> HighlightStyle {
    HighlightStyle {
...
pub fn block_quote_style(color: Hsla) -> HighlightStyle {
    HighlightStyle {
        color: Some(color),
        ..HighlightStyle::default()
    }
...
pub fn completed_task_style(color: Hsla) -> HighlightStyle {
    HighlightStyle {
        color: Some(color),
        ..HighlightStyle::default()
    }
...
pub fn strikethrough_style() -> HighlightStyle {
    HighlightStyle {
...
pub fn italic_style() -> HighlightStyle {
    HighlightStyle {
...
pub fn style_heading(text: &str, appearance: Appearance) -> Option<YmdStyledHeading> {
    ...
    let mut merged_conceal_ranges: Vec<Range<usize>> = Vec::new();
    for range in conceal_ranges {
        if let Some(last) = merged_conceal_ranges.last_mut()
            && range.start <= last.end
        {
            last.end = last.end.max(range.end);
            continue;
        }
        merged_conceal_ranges.push(range);
    }

    let mut visible_source_ranges = Vec::new();
    let mut cursor = 0;
    for range in &merged_conceal_ranges {
        if cursor < range.start {
            visible_source_ranges.push(cursor..range.start);
        }
        cursor = range.end;
    }
    if cursor < text.len() {
        visible_source_ranges.push(cursor..text.len());
    }

    let styled_text = visible_source_ranges
        .iter()
        .map(|range| &text[range.clone()])
        .collect::<String>();

    let (styled_text, visible_source_ranges) = if styled_text.trim().is_empty() {
        (text.into(), vec![0..text.len()])
    } else {
        (styled_text, visible_source_ranges)
    };

    Some(YmdStyledHeading {
        text: styled_text.into(),
        foreground_style: line_foreground_style(color, appearance),
        visible_source_ranges,
        source_len: text.len(),
    })
}

pub fn scan_background_markups(text: &str) -> Vec<YmdBackgroundMarkup> {
    ...
    for_each_line(text, |line_content, line_start| {
        ...
        markups.extend(
            scan_line_background_markups(line_content, line_start, &inline_code_ranges)
                .into_iter()
                .map(|inline_highlight| YmdBackgroundMarkup {
                    range: inline_highlight.range,
                    open_marker_range: inline_highlight.open_marker_range,
                    close_marker_range: inline_highlight.close_marker_range,
                    color: inline_highlight.color,
                }),
        );
    });
...
pub fn scan_links(text: &str) -> Vec<YmdLink> {
...
pub fn markdown_link_url_at_offset(text: &str, offset: usize) -> Option<(Range<usize>, String)> {
...
pub fn scan_images(text: &str) -> Vec<YmdImage> {
...
pub fn scan_block_quotes(text: &str) -> Vec<YmdBlockQuote> {
...
pub fn scan_checkboxes(text: &str) -> Vec<YmdCheckbox> {
...
pub fn scan_checked_task_text_ranges(text: &str) -> Vec<Range<usize>> {
...
pub fn scan_strikethroughs(text: &str) -> Vec<Range<usize>> {
    scan_inline_style_content_ranges(text, "~~")
}

pub fn scan_italics(text: &str) -> Vec<Range<usize>> {
    scan_inline_style_content_ranges(text, "*")
}

fn scan_inline_style_content_ranges(text: &str, marker: &str) -> Vec<Range<usize>> {
    let mut ranges = Vec::new();
    let fenced_code_ranges = fenced_code_block_ranges(text);
    ...
    for_each_line(text, |line_content, line_start| {
        ...
        ranges.extend(
            scan_line_inline_styles(line_content, line_start)
                .into_iter()
                .filter(|inline_style| inline_style.marker == marker)
...
pub fn is_escaped_at(line: &str, index: usize) -> bool {
    let bytes = line.as_bytes();
...
pub fn is_inside_inline_code_at(line: &str, index: usize) -> bool {
    let bytes = line.as_bytes();
    let mut cursor = 0;
...
mod tests {
    ...
    #[test]
    fn styles_heading_for_outline_surfaces() {
        let heading = "## 🟢⋯ Green Heading";
        let styled = style_heading(heading, Appearance::Light).unwrap();

        assert_eq!(styled.text.as_ref(), "⋯ Green Heading");
        assert_eq!(
            styled.foreground_style,
            line_foreground_style(YmdColor::Green, Appearance::Light)
        );
        assert_eq!(styled.rendered_range_for_source_range(0..2), None);
        assert_eq!(styled.rendered_range_for_source_range(7..10), Some(0..3));
        assert_eq!(
            styled.rendered_range_for_source_range(10..heading.len()),
            Some(3..styled.text.len())
        );
    }
    ...
    fn style_heading_conceals_mid_text_color_marker_like_the_editor() {
        ...
        assert_eq!(styled.text.as_ref(), "Blue  Heading");
        assert_eq!(styled.rendered_range_for_source_range(0..2), None);
        assert_eq!(styled.rendered_range_for_source_range(2..6), Some(0..4));
        assert_eq!(styled.rendered_range_for_source_range(7..11), None);
        assert_eq!(
            styled.rendered_range_for_source_range(12..heading.len()),
            Some(6..13)
        );
    }
    ...
    #[test]
    fn style_heading_keeps_emoji_only_headings_navigable() {
        let heading = "## 🟢";
        let styled = style_heading(heading, Appearance::Light).unwrap();

        assert_eq!(styled.text.as_ref(), heading);
        assert_eq!(
            styled.rendered_range_for_source_range(0..heading.len()),
            Some(0..heading.len())
        );
    }
    ...
    #[test]
    fn scans_italic_content_ranges() {
        assert_eq!(italic_style().font_style, Some(FontStyle::Italic));
        assert_eq!(scan_italics("*italic*"), vec![1..7]);
        assert_eq!(
            scan_italics("plain *italic*\n**bold**\n`*literal*`\n```md\n*code*\n```"),
            vec![7..13]
        );
    }
