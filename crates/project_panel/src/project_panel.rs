...
use gpui::{
    ...
    DragMoveEvent, ElementId, Empty, Entity, EventEmitter, ExternalDragPayload, ExternalPaths,
    FileDragPaths, FocusHandle, Focusable, FontWeight, Hsla, InteractiveElement, KeyContext,
    ListHorizontalSizingBehavior, ListSizingBehavior, Modifiers, ModifiersChangedEvent,
    MouseButton, MouseDownEvent, MouseExitEvent, MouseUpEvent, ParentElement, PathPromptOptions,
...
impl ProjectPanel {
    ...
    fn register_tagged_paths_for_scan(&mut self, cx: &mut Context<Self>) {
        let mut paths_by_worktree: HashMap<WorktreeId, Vec<Arc<RelPath>>> = HashMap::default();
        {
            ...
            for file_tag_key in self.file_tags.tags.keys() {
                let Ok(path) = RelPath::from_unix_str(file_tag_key.path.as_str()) else {
                    continue;
                };
    ...
    fn tags_header_height(window: &Window, cx: &App) -> Pixels {
        ui::ListHeader::default_height(cx).to_pixels(window.rem_size())
    }
